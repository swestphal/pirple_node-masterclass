                ...
                _data.read('users', phone, function (err, userData) {
                    if (!err && data) {
                        _data.delete('users', phone, function (err) {
                            if (!err) {
                                // delete each of checks associated with user
                                        ...
                                        _data.delete('checks', checkId, function (err) {
                                            ...
                                            if (checksDeleted == checksToDelete) {
                                                if (!deletionErrors) {
                                                    ...
                                                    callback(500, { 'Error': 'Errors encountered when delete all checks of deleted user' })
                                                }
                                            }
                                        })
                                    });
...
handlers._tokens.post = function (data, callback) {
    ...
    const password =
        typeof data.payload.password == 'string' &&
            data.payload.password.trim().length > 4
            ? data.payload.password.trim()
            : false;
...
// checks - post
// required field : protocol, url, method, successCodes, timeoutSeconds
// optional field : none
...
handlers._checks.post = function (data, callback) {
    ...
    if (protocol && url && method && successCodes && timeoutSeconds) {
                ...
                _data.read('users', userPhone, function (err, userData) {
                    if (!err && userData) {
                        ...
                        if (userChecks.length < config.maxChecks) {
                            ...
                            callback(400, {
                                Error:
                                    'The user already have the max number of checks (' +
                                    config.maxChecks +
                                    ')',
                            });
                        }
        ...
        callback(400, {
            Error: 'Missing required input, or inputs are invalid',
        });
    }
};
            ...
            if (!err && checkData) {
                ...
                const token =
                    typeof data.headers.token === 'string'
                        ? data.headers.token
                        : false;
                // verify that given token is valid for user that created the check
...
// checks - put
// required field : id
// optional field : protocol, url, method successCodes, timeoutSeconds (one at least)
            ...
            _data.read('checks', id, function (err, checkData) {
                if (!err && checkData) {
                    ...
                    handlers._tokens.verifyToken(
                        ...
                        function (tokenIsValid) {
                            if (tokenIsValid) {
                                // update the chech where necessary
                                if (protocol) {
                    ...
                    callback(400, { Error: 'Check did not exist' });
                }
            });
        } else {
            callback(400, { Error: 'Missing fields to update' });
    ...
    if (id) {
        ...
        _data.read('checks', id, function (err, checkData) {
            if (!err && checkData) {
                                ...
                                if (!err) {
                                    ...
                                    _data.read(
                                        ...
                                        function (err, userData) {
                                            if (!err && userData) {
                                                const userChecks = typeof (userData.checks) === 'object' && userData.checks instanceof Array ? userData.checks : [];
                                                // remove delete check from their list of checks
                                                    ...
                                                    _data.update(
                                                        ...
                                                        function (err) {
                                                            if (!err) {
                                                                ...
                                                                callback(500, {
                                                                    Error:
                                                                        "Couldn't update specified user",
                                                                });
                                                            }
                                                        }
                                                    );
                                                } else {
                                                    callback(500, { 'Error': 'Could not find the check on the users oject. Couldn remove it' })
                                                }

                                            } else {
                                                callback(500, {
                                                    Error:
                                                        "Couldn't not find the user who created the check, so couldnt remove the checks from users object",
                                                });
                                            }
                                        }
                                    );
                ...
                callback(400, {
                    Error: 'The specified chech ID does not exist',
                });
            }
        });
        // get the token from the headers
